...
compute_d_prime <- function(data, identity_type) {
  # 计算命中、误报、漏报、正确反应的数量
  hit <- length(data$ACC[data$Matching == "Matching" & data$Identity == identity_type & data$ACC == 1])
  fa <- length(data$ACC[data$Matching == "Nonmatching" & data$Identity == identity_type & data$ACC == 0])
  miss <- length(data$ACC[data$Matching == "Matching" & data$Identity != identity_type & data$ACC == 0])
  cr <- length(data$ACC[data$Matching == "Nonmatching" & data$Identity != identity_type& data$ACC == 1])
