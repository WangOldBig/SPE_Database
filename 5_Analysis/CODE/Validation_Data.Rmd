...
```{r}
# 定义计算d'的函数
calculate_d_prime <- function(hit_rate, fa_rate, miss_rate, cr_rate) {
...
compute_d_prime <- function(data, matching_type, identity_type) {
  d_prime <- data %>%
    ...
    summarize(
      hit_rate = mean(ACC[ACC == 1 & Matching == matching_type & Identity == identity_type]),
      fa_rate = mean(ACC[ACC == 0 & Matching != matching_type & Identity == identity_type]),
      miss_rate = mean(ACC[ACC == 0 & Matching == matching_type & Identity == identity_type]),
      cr_rate = mean(ACC[ACC == 1 & Matching != matching_type & Identity == identity_type])
    ) %>%
    mutate(d_prime = calculate_d_prime(hit_rate, fa_rate, miss_rate, cr_rate))
  
  if (sum(is.na(d_prime)) > 0) {
    return(NA)  # 如果计算结果中存在NA，则返回NA
  } else {
    return(d_prime$d_prime)
  }
}
...
df_dp <- function(df, i) {
  result <- df %>%
    ...
    dplyr::summarise(
      hit = sum(ACC[Matching == "Matching" & ACC == 1]),
      fa = sum(ACC[Matching == "Nonmatching" & ACC == 0]),
      miss = sum(ACC[Matching == "Matching" & ACC == 0]),
      cr = sum(ACC[Matching == "Nonmatching" & ACC == 1]),
      Dprime = qnorm(ifelse(hit / (hit + miss) < 1, hit / (hit + miss), 1 - 1 / (2 * (hit + miss)))) -
               qnorm(ifelse(fa / (fa + cr) > 0, fa / (fa + cr), 1 / (2 * (fa + cr))))
    )
  return(result)
}
```
...
df_dp <- function(df) {
  result <- df %>%
    ...
    mutate(
      ...
      fa_rate = fa / (fa + cr),
      d_prime = calculate_d_prime(hit_rate, fa_rate)
    ) %>%
    select(Subject, Session, Identity, d_prime)
