...
```{r}
# 定义函数
combine_data <- function(df, key_columns) {
  # 提取关键列
...
for (i in 1:20) {
  if (!is.null(df[[i]])) {
    ...
    cat(
      ...
      ifelse(nrow(min_ACC) > 0, min(min_ACC$mean_ACC), "No data"), # Check if min_ACC is empty
      "\n"
    )
  } else {
    cat("df[[", i, "]] is empty.\n")
...
for (i in 1:20) {
  min_ACC <- df[[i]] %>%
    dplyr::group_by(Subject, Matching, Identity) %>%
    #dplyr::group_by(Subject) %>%
    dplyr::summarise(mean_ACC = mean(ACC)) %>%
    dplyr::ungroup() %>%
    dplyr::filter(mean_ACC < 0.5) %>%
