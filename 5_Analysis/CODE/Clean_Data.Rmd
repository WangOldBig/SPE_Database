...
```{r}
combine_data <- function(df_list, key_columns) {
  combined_data <- data.frame()
  
  for (i in seq_along(df_list)) {
    key_data <- df_list[[i]][, key_columns]
    combined_data <- bind_rows(combined_data, key_data)
  }
  
  return(combined_data)
}
# 示例调用函数
key_columns <- c("Subject","Matching","Identity","RT_ms","ACC")  # 替换为您想要提取的关键列
combined_data <- combine_data(df, key_columns)
```
...
for (i in seq_along(df)) {
  if (!is.null(df[[i]])) {
    ...
    write.csv(
      x = filtered_df,
      file = paste0("./DATA/df_", i, "_filtered.csv"),
      row.names = FALSE
    )
    
    cat("Filtered data for df[[", i, "]] has been saved to ./DATA/df_", i, "_filtered.csv\n")
  } else {
    cat("df[[", i, "]] is empty.\n")
  }
}
