...
## Experiment 1 (Origin_Identity = 2)
"Self", "Stranger"
...
df_pocess <- df[[1]] %>%
  dplyr::filter(Origin_Identity %in% c("Self", "Stranger"))

df_pocess$Origin_Identity <- factor(df_pocess$Origin_Identity, levels = c("Self", "Stranger"))

# 计算每个被试的Cohen's d RT
df_RT <- df_pocess %>%
  dplyr::group_by(Subject, Matching) %>%
  dplyr::reframe(
    Cohens_d_RT = cohen.d(RT_ms ~ Origin_Identity)$estimate,  # 计算Cohen's d
    deta_RT = mean(RT_ms[Origin_Identity == "Self"], na.rm = TRUE) - 
      mean(RT_ms[Origin_Identity == "Stranger"], na.rm = TRUE)  # 计算RT差值
  )

df_RT <- df_pocess %>%
  dplyr::group_by(Subject, Matching) %>%  # 添加Matching作为分组变量
  dplyr::reframe(
    Cohens_d_RT = cohen.d(RT_ms ~ Origin_Identity)$estimate,  # 计算Cohen's d
    deta_RT = mean(RT_ms[Origin_Identity == "Self"], na.rm = TRUE) - 
              mean(RT_ms[Origin_Identity == "Stranger"], na.rm = TRUE)  # 计算RT差值
...
df_ACC <- df_pocess %>%
  dplyr::group_by(Subject, Matching) %>%
  dplyr::reframe(
    Cohens_d_ACC = cohen.d(ACC[ACC %in% c(0, 1)] ~ Origin_Identity[ACC %in% c(0, 1)])$estimate,  # 计算Cohen's d，只考虑0和1
    deta_ACC = mean(ACC[Origin_Identity == "Self" & ACC %in% c(0, 1)], na.rm = TRUE) - 
               mean(ACC[Origin_Identity == "Stranger" & ACC %in% c(0, 1)], na.rm = TRUE)  # 计算ACC差值，忽略3和NA
  )
...
## Experiment 2 (Origin_Identity = 2)
"Self", "Stranger"
...
df_RT <- df_pocess %>%
  ...
  dplyr::reframe(
    Cohens_d_RT = cohen.d(RT_ms ~ Origin_Identity)$estimate,  # 计算Cohen's d
    deta_RT = mean(RT_ms[Origin_Identity == "Self"], na.rm = TRUE) - mean(RT_ms[Origin_Identity == "Stranger"], na.rm = TRUE)  # 计算RT差值
  )
...
## Experiment 1 (Origin_Identity = 3)
...
df_pocess <- df[[3]] %>%
  dplyr::filter(Origin_Identity %in% c("Self", "Familiar"))

df_pocess$Origin_Identity <- factor(df_pocess$Origin_Identity, levels = c("Self", "Familiar"))
...
df_RT <- df_pocess %>%
  ...
  dplyr::reframe(
    Cohens_d_RT = cohen.d(RT_ms ~ Origin_Identity)$estimate,  # 计算Cohen's d
    deta_RT = mean(RT_ms[Origin_Identity == "Self"], na.rm = TRUE) - mean(RT_ms[Origin_Identity == "Familiar"], na.rm = TRUE)  # 计算RT差值
  )
...
df_ACC <- df_pocess %>%
  ...
  dplyr::reframe(
    Cohens_d_ACC = cohen.d(ACC[ACC %in% c(0, 1)] ~ Origin_Identity[ACC %in% c(0, 1)])$estimate,  # 计算Cohen's d，只考虑0和1
    deta_ACC = mean(ACC[Origin_Identity == "Self" & ACC %in% c(0, 1)], na.rm = TRUE) - 
      mean(ACC[Origin_Identity == "Familiar" & ACC %in% c(0, 1)], na.rm = TRUE)  # 计算ACC差值，忽略3和NA
  )
...
## Experiment 1 (Origin_Identity = 4)
...
df[[5]] <- read.csv(
  ...
  dplyr::select(
    ...
    Matching, Origin_Identity, RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
...
# 提取Self和Mother的反应时间
df_pocess <- df[[4]] %>%
  dplyr::filter(Origin_Identity %in% c("Self", "Mother"))

df_pocess$Origin_Identity <- factor(df_pocess$Origin_Identity, levels = c("Self", "Mother"))
...
df_RT <- df_pocess %>%
  ...
  dplyr::reframe(
    Cohens_d_RT = cohen.d(RT_ms ~ Origin_Identity)$estimate,  # 计算Cohen's d
    deta_RT = mean(RT_ms[Origin_Identity == "Self"], na.rm = TRUE) - mean(RT_ms[Origin_Identity == "Mother"], na.rm = TRUE)  # 计算RT差值
  )
...
df_ACC <- df_pocess %>%
  ...
  dplyr::reframe(
    Cohens_d_ACC = cohen.d(ACC[ACC %in% c(0, 1)] ~ Origin_Identity[ACC %in% c(0, 1)])$estimate,  # 计算Cohen's d，只考虑0和1
    deta_ACC = mean(ACC[Origin_Identity == "Self" & ACC %in% c(0, 1)], na.rm = TRUE) - 
      mean(ACC[Origin_Identity == "Mother" & ACC %in% c(0, 1)], na.rm = TRUE)  # 计算ACC差值，忽略3和NA
  )
...
# 提取Self和none的反应时间
df_pocess <- df[[4]] %>%
  dplyr::filter(Origin_Identity %in% c("Self", "none"))

df_pocess$Origin_Identity <- factor(df_pocess$Origin_Identity, levels = c("Self", "none"))
...
df_RT <- df_pocess %>%
  ...
  dplyr::reframe(
    Cohens_d_RT = cohen.d(RT_ms ~ Origin_Identity)$estimate,  # 计算Cohen's d
    deta_RT = mean(RT_ms[Origin_Identity == "Self"], na.rm = TRUE) - mean(RT_ms[Origin_Identity == "none"], na.rm = TRUE)  # 计算RT差值
  )
...
df_ACC <- df_pocess %>%
  ...
  dplyr::reframe(
    Cohens_d_ACC = cohen.d(ACC[ACC %in% c(0, 1)] ~ Origin_Identity[ACC %in% c(0, 1)])$estimate,  # 计算Cohen's d，只考虑0和1
    deta_ACC = mean(ACC[Origin_Identity == "Self" & ACC %in% c(0, 1)], na.rm = TRUE) - 
      mean(ACC[Origin_Identity == "none" & ACC %in% c(0, 1)], na.rm = TRUE)  # 计算ACC差值，忽略3和NA
  )
...
df_pocess <- df[[4]] %>%
  dplyr::filter(Origin_Identity %in% c("Self", "Acquaintance"))

df_pocess$Origin_Identity <- factor(df_pocess$Origin_Identity, levels = c("Self", "Acquaintance"))
...
df_RT <- df_pocess %>%
  ...
  dplyr::reframe(
    Cohens_d_RT = cohen.d(RT_ms ~ Origin_Identity)$estimate,  # 计算Cohen's d
    deta_RT = mean(RT_ms[Origin_Identity == "Self"], na.rm = TRUE) - mean(RT_ms[Origin_Identity == "Acquaintance"], na.rm = TRUE)  # 计算RT差值
  )
...
df_ACC <- df_pocess %>%
  ...
  dplyr::reframe(
    Cohens_d_ACC = cohen.d(ACC[ACC %in% c(0, 1)] ~ Origin_Identity[ACC %in% c(0, 1)])$estimate,  # 计算Cohen's d，只考虑0和1
    deta_ACC = mean(ACC[Origin_Identity == "Self" & ACC %in% c(0, 1)], na.rm = TRUE) - 
      mean(ACC[Origin_Identity == "Acquaintance" & ACC %in% c(0, 1)], na.rm = TRUE)  # 计算ACC差值，忽略3和NA
  )
...
## Experiment 3 (Origin_Identity = 3)
...
# 提取Self和Friend的反应时间
df_pocess <- df[[7]] %>%
  dplyr::filter(Origin_Identity %in% c("Self", "Friend"))

df_pocess$Origin_Identity <- factor(df_pocess$Origin_Identity, levels = c("Self", "Friend"))
...
df_RT <- df_pocess %>%
  ...
  dplyr::reframe(
    Cohens_d_RT = cohen.d(RT_ms ~ Origin_Identity)$estimate,  # 计算Cohen's d
    deta_RT = mean(RT_ms[Origin_Identity == "Self"], na.rm = TRUE) - mean(RT_ms[Origin_Identity == "Friend"], na.rm = TRUE)  # 计算RT差值
  )
...
df_ACC <- df_pocess %>%
  ...
  dplyr::reframe(
    Cohens_d_ACC = cohen.d(ACC[ACC %in% c(0, 1)] ~ Origin_Identity[ACC %in% c(0, 1)])$estimate,  # 计算Cohen's d，只考虑0和1
    deta_ACC = mean(ACC[Origin_Identity == "Self" & ACC %in% c(0, 1)], na.rm = TRUE) - 
      mean(ACC[Origin_Identity == "Friend" & ACC %in% c(0, 1)], na.rm = TRUE)  # 计算ACC差值，忽略3和NA
  )
...
# 将合并后的列表存储到 df_Self_Stranger 中
df_Self_Stranger[[4]] <- merged_list


names(df_Self_Stranger)[4] <- "Wozniak_2018_PLOS Exp1"

rm(df_RT,df_ACC,merged_list,df_pocess)
```

## Experiment 2 (Origin_Identity = 3)
"Self", "Friend", "Stranger"
```{r Paper p95 Experiment 2}
df[[8]] <- read.csv(
  "../1_Clean_Data/Wozniak_2018_PLOS/Wozniak_2018_PLOS_Exp2_Clean.csv"
  ) %>%
   dplyr::mutate(
    ScreenResolution = "24’’ widescreen LED monito", # 屏幕分辨率
    RefreshRate = NA, # 文档中未提及刷新率
  ...
  # 更新列名
  names(df)[8] <- "Wozniak_2018_PLOS_Exp2"
```
...
# 将合并后的列表存储到 df_Self_Friend 中
df_Self_Friend[[2]] <- merged_list


names(df_Self_Friend)[2] <- "Wozniak_2018_PLOS Exp2"
...
d_primes <- setNames(data.frame(Origin_Identity = character(), d_prime = numeric()), names(identities))
## 计算整篇文章的d'
# 创建一个新的d_prime数据框，用于保存d_prime值
df_d_prime[[11]] <- data.frame(Origin_Identity = character(), d_prime = numeric())

# 计算d'
for (Origin_Identity in identities) {
  # 提取当前Origin_Identity的数据
  subset_data <- df[[11]] %>% filter(Origin_Identity == Origin_Identity)
  ...
  df_d_prime[[11]] <- rbind(df_d_prime[[11]], data.frame(Origin_Identity = Origin_Identity, d_prime = d_prime))
}

# 打印df_d_prime[[11]]，包含所有Origin_Identity的d'值
print(df_d_prime[[11]])
...
##计算每个被试的d'
d_primes <- setNames(data.frame(Subject = character(), Origin_Identity = character(), d_prime = numeric()), c("Subject", "Origin_Identity", "d_prime"))

# 创建一个新的d_prime数据框，用于保存d_prime值
df_d_paper[[11]] <- data.frame(Subject = character(), Origin_Identity = character(), d_prime = numeric())
...
for (Origin_Identity in identities) {
  ...
  for (subject in subjects) {
    # 提取当前Origin_Identity和Subject的数据
    subset_data <- df[[11]] %>% filter(Origin_Identity == Origin_Identity & Subject == subject)
    ...
    # 保存结果
    d_primes <- rbind(d_primes, data.frame(Subject = subject, Origin_Identity = Origin_Identity, d_prime = d_prime))
    # 保存计算结果到df_d_paper[[11]]
    df_d_paper[[11]] <- rbind(df_d_paper[[11]], data.frame(Subject = subject, Origin_Identity = Origin_Identity, d_prime = d_prime))
  }
}

# 打印df_d_paper[[11]]，包含所有Subject和Origin_Identity的d'值
print(df_d_paper[[11]])
# 输出到新的CSV文件
write.csv(df_d_paper[[11]], "../1_Clean_Data/d_prime/Golubickis_2021_AP_Exp1_d'.csv", row.names = FALSE)
# 显示数据总结
summary(df[[11]]) 
rm(d_primes,subset_data,correct_rejection,d_prime,false_alarm,false_alarm_rate,hit,hit_rate,identities,Origin_Identity,miss,z_false_alarm,z_hit,subject,subjects)
```
...
## 计算整篇文章的d'
# 创建一个新的d_prime数据框，用于保存d_prime值
df_d_prime[[12]] <- data.frame(Origin_Identity = character(), d_prime = numeric())

# 计算d'
for (Origin_Identity in identities) {
  # 提取当前Origin_Identity的数据
  subset_data <- df[[12]] %>% filter(Origin_Identity == Origin_Identity)
  ...
  df_d_prime[[12]] <- rbind(df_d_prime[[12]], data.frame(Origin_Identity = Origin_Identity, d_prime = d_prime))
}

# 打印df_d_prime[[12]]，包含所有Origin_Identity的d'值
print(df_d_prime[[12]])
...
# 创建一个新的d_prime数据框，用于保存d_prime值
df_d_paper[[12]] <- data.frame(Subject = character(), Origin_Identity = character(), d_prime = numeric())
...
for (Origin_Identity in identities) {
  ...
  for (subject in subjects) {
    # 提取当前Origin_Identity和Subject的数据
    subset_data <- df[[12]] %>% filter(Origin_Identity == Origin_Identity & Subject == subject)
    ...
    df_d_paper[[12]] <- rbind(df_d_paper[[12]], data.frame(Subject = subject, Origin_Identity = Origin_Identity, d_prime = d_prime))
  }
}

# 打印df_d_paper[[12]]，包含所有Subject和Origin_Identity的d'值
print(df_d_paper[[12]])
# 输出到新的CSV文件
...
## 计算整篇文章的d'
# 创建一个新的d_prime数据框，用于保存d_prime值
df_d_prime[[13]] <- data.frame(Origin_Identity = character(), d_prime = numeric())

# 计算d'
for (Origin_Identity in identities) {
  # 提取当前Origin_Identity的数据
  subset_data <- df[[13]] %>% filter(Origin_Identity == Origin_Identity)
  ...
  df_d_prime[[13]] <- rbind(df_d_prime[[13]], data.frame(Origin_Identity = Origin_Identity, d_prime = d_prime))
}

# 打印df_d_prime[[13]]，包含所有Origin_Identity的d'值
print(df_d_prime[[13]])
...
# 创建一个新的d_prime数据框，用于保存d_prime值
df_d_paper[[13]] <- data.frame(Subject = character(), Origin_Identity = character(), d_prime = numeric())
...
for (Origin_Identity in identities) {
  ...
  for (subject in subjects) {
    # 提取当前Origin_Identity和Subject的数据
    subset_data <- df[[13]] %>% filter(Origin_Identity == Origin_Identity & Subject == subject)
    ...
    df_d_paper[[13]] <- rbind(df_d_paper[[13]], data.frame(Subject = subject, Origin_Identity = Origin_Identity, d_prime = d_prime))
  }
}

# 打印df_d_paper[[13]]，包含所有Subject和Origin_Identity的d'值
print(df_d_paper[[13]])
# 输出到新的CSV文件
...
df[[14]] <- read.csv(
  ...
  dplyr::arrange( 
    Subject, Origin_Identity, Matching
  )
...
## 计算整篇文章的d'
# 创建一个新的d_prime数据框，用于保存d_prime值
df_d_prime[[14]] <- data.frame(Origin_Identity = character(), d_prime = numeric())

# 计算d'
for (Origin_Identity in identities) {
  # 提取当前Origin_Identity的数据
  subset_data <- df[[14]] %>% filter(Origin_Identity == Origin_Identity)
  ...
  df_d_prime[[14]] <- rbind(df_d_prime[[14]], data.frame(Origin_Identity = Origin_Identity, d_prime = d_prime))
}

# 打印df_d_prime[[14]]，包含所有Origin_Identity的d'值
print(df_d_prime[[14]])
...
# 创建一个新的d_prime数据框，用于保存d_prime值
df_d_paper[[14]] <- data.frame(Subject = character(), Origin_Identity = character(), d_prime = numeric())
...
for (Origin_Identity in identities) {
  ...
  for (subject in subjects) {
    # 提取当前Origin_Identity和Subject的数据
    subset_data <- df[[14]] %>% filter(Origin_Identity == Origin_Identity & Subject == subject)
    ...
    df_d_paper[[14]] <- rbind(df_d_paper[[14]], data.frame(Subject = subject, Origin_Identity = Origin_Identity, d_prime = d_prime))
  }
}

# 打印df_d_paper[[14]]，包含所有Subject和Origin_Identity的d'值
print(df_d_paper[[14]])
# 输出到新的CSV文件
...
df[[15]] <- read.csv(
  ...
  dplyr::mutate(
    ...
    Origin_Identity = factor(
      Origin_Identity, levels = c("Self", "Father", "Stranger")
    ),
  ...
  dplyr::select(
    ...
    Matching, Origin_Identity, Response, RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
...
## Experiment 4 (Origin_Identity = 3)
"Self", "Friend", "Stranger"
...
df[[16]] <- read.csv(
  ...
  dplyr::select(
    Subject, Gender, Age, Handedness, First_Lanugage, 
    Shape, Word, Matching, Origin_Identity, Response, RT_ms, RT_sec, ACC
    ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
...
df[[17]] <- read.csv(
  ...
  dplyr::mutate(
    ...
    Origin_Identity = factor(
      Origin_Identity, levels = c("Self", "Friend")
      ),
    RT_ms = as.numeric(RT_ms),
    RT_sec = as.numeric(RT_sec),
    ACC = as.numeric(ACC),
  ...
  dplyr::select(
    Subject, Word, Shape, Matching, Origin_Identity, RT_ms, RT_sec, ACC
    ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
...
df[[18]] <- read.csv(
  ...
  dplyr::select(
    Subject, Word, Matching, Origin_Identity, RT_ms, RT_sec, ACC
    ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
...
## Experiment 3 (Origin_Identity = 2)
"Self", "Friend"
...
df[[20]] <- read.csv(
  ...
  dplyr::select(
    Subject, Shape, Word, Matching, Origin_Identity, Group, RT_ms, RT_sec, ACC
    ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
  ...
  dplyr::arrange(
    Subject, Matching, Origin_Identity, Group
    )
...
df[[21]] <- read.csv(
  ...
  dplyr::mutate(
    ...
    Origin_Identity = factor(
      Origin_Identity, levels = c("Self", "Stranger")
    ),
    RT_ms = as.numeric(RT_ms),
    RT_sec = as.numeric(RT_sec),
    ACC = as.numeric(ACC),
  ...
  dplyr::select(
    Subject, Matching, Origin_Identity,RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
...
```
# Constable_2019_EPHPP
## Experiment 2 (Origin_Identity = 4)
"Self", "We", "They","Alone"
...
```
# Constable_2019_EPHPP
## Experiment 3 (Origin_Identity = 4)
"Self", "We", "They","Alone"
...
```
# Constable_2019_EPHPP
## Experiment 4 (Origin_Identity = 5)
"Self", "We", "They","Alone"
...
df[[26]] <- read.csv(
  ...
  dplyr::select(
    Subject, Age, Handedness, Gender, Response, Symmetry, Matching, Origin_Identity, RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
...
df[[28]] <- read.csv(
  ...
  dplyr::select(
    ...
    StrangerShape, Matching, Origin_Identity, RT_ms, RT_sec, ACC
    ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
  ...
  dplyr::arrange(
    Subject, Phase, Matching, Origin_Identity
    )
...
df[[29]] <- read.csv(
  ...
  dplyr::select(
    ...
    Shape, Label,  Matching, Origin_Identity, Morality,
  ...
  dplyr::arrange(
    Subject, Matching, Origin_Identity, Morality
    )
...
df[[31]] <- read.csv("../1_Clean_Data/Woźniak_2022_PR/Woźniak_2022_PR_Exp1_Clean.csv") %>%
  ...
  dplyr::mutate(
    ...
    Origin_Identity = factor(
      Origin_Identity, levels = c("Self", "Friend", "None")
    ),
    RT_ms = as.numeric(RT_ms),
    RT_sec = as.numeric(RT_sec),
    ACC = as.numeric(ACC),
  ...
  dplyr::select(
    Subject, Gender, 
    Shape, Label1, Label2, Label3, Origin_Identity,  
...
df[[34]] <- read.csv(
  ...
  dplyr::select(
    Subject, Age, Gender, Handedness,
    Shape, Label,  Matching, Origin_Identity,
...
df[[36]] <- temp %>%
  ...
  dplyr::select(
    ...
    Treatment,
    Shape, Matching, Origin_Identity, RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
...
df[[37]] <- read.csv(
  ...
  dplyr::select(
    ...
    Sadshapeother, Sadshapeself, Othershape, Selfshape, Label, Origin_Identity,
    Matching, Response, RT_ms, RT_sec, ACC
    ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
...
df[[38]] <- read.csv(
  ...
  dplyr::select(
    Subject, Age, Gender, 
    Shape, Matching, Origin_Identity, Response, Phase, RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
...
df[[40]] <- df_Ps1_E1 %>%
  ...
  dplyr::select(
    Subject, Matching, Label, Response, Origin_Identity, RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
...
"../1_Clean_Data/Orellana-Corrales_2021_EP/Orellana-Corrales_2021_EP_Exp1_Clean.csv"
  ...
  dplyr::select(
    ...
    Shape1 = S1,
    Shape2 = S2,
    Origin_Identity1 = ID1,
    Origin_Identity2 = ID2,
  ...
  dplyr::select(
    Subject, Age, Gender, Handedness, 
    Shape, Label, Shape1, Shape2, Matching, Origin_Identity1, Origin_Identity2, 
    Response, RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
...
"../1_Clean_Data/Orellana-Corrales_2021_EP/Orellana-Corrales_2021_EP_Exp2_Clean.csv"
  ...
  dplyr::select(
    Subject, Age, Gender, Handedness, 
    Shape, Label, Matching, Shape1, Shape2, Origin_Identity1, Origin_Identity2, 
    Response, RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
...
combine_data <- function(data_list) {
  ...
  for (i in seq_along(data_list)) {
    # 提取关键列
    key_columns <- data_list[[i]][c("Subject", "Matching", "Origin_Identity", "RT_ms", "RT_sec", "ACC")]
...
df[[22]] <- read.csv(
  ...
  dplyr::select(
    Subject, Matching, Origin_Identity, Session, RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
