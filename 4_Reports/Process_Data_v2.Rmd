...
# 提取Self和Stranger的反应时间
df_Self_Stranger <- df[[2]] %>%
  dplyr::filter(Identity %in% c("Self", "Stranger"))

# 计算每个被试的Cohen's d RT
df_RT <- df_Self_Stranger %>%
  dplyr::group_by(Subject) %>%
  dplyr::reframe(
    Cohens_d = cohen.d(RT_ms ~ Identity)$estimate,  # 计算Cohen's d
    deta_RT = mean(RT_ms[Identity == "Self"], na.rm = TRUE) - mean(RT_ms[Identity == "Stranger"], na.rm = TRUE)  # 计算RT差值
  )


# 计算每个被试的Cohen's d ACC
df_ACC <- df_Self_Stranger %>%
  dplyr::group_by(Subject) %>%
  dplyr::reframe(
    Cohens_d = cohen.d(ACC[ACC %in% c(0, 1)] ~ Identity[ACC %in% c(0, 1)])$estimate,  # 计算Cohen's d，只考虑0和1
    deta_ACC = mean(ACC[Identity == "Self" & ACC %in% c(0, 1)], na.rm = TRUE) - 
               mean(ACC[Identity == "Stranger" & ACC %in% c(0, 1)], na.rm = TRUE)  # 计算ACC差值，忽略3和NA
  )
...
# 将结果添加到列表中
df_Self_Stranger_CohenD_RT[[2]] <- df_RT
df_Self_Stranger_CohenD_ACC[[2]] <- df_ACC

names(df_Self_Stranger_CohenD_ACC)[2] <- "Constable_2020_CE Exp 2"
names(df_Self_Stranger_CohenD_RT)[2] <- "Constable_2020_CE Exp 2"
names(df)[2] <- "Constable_2020_CE Exp 2"

summary(df_Self_Stranger_CohenD_ACC[[2]])
summary(df_Self_Stranger_CohenD_RT[[2]])

rm(df_Self_Stranger,df_RT,df_ACC)
```
...
# 将结果添加到列表中
df_Self_Stranger_CohenD_RT[[3]] <- df_RT
df_Self_Stranger_CohenD_ACC[[3]] <- df_ACC

names(df_Self_Stranger_CohenD_ACC)[3] <- "Haoyue Qian_2019_QJEP Exp 1"
names(df_Self_Stranger_CohenD_RT)[3] <- "Haoyue Qian_2019_QJEP Exp 1"
names(df)[3] <- "Haoyue Qian_2019_QJEP Exp 1"

summary(df_Self_Stranger_CohenD_ACC[[3]])
summary(df_Self_Stranger_CohenD_RT[[3]])
