...
# 为Self, Friend，Stranger分别计算
  ...
  d_primes <- setNames(data.frame(Identity = character(), d_prime = numeric()), names(identities))
    ...
    # 更新列名
    names(df)[6] <- "Schaefer_2019_CP_Exp1"
  ...
  ##计算每个被试的d'
    ...
    print(df_d_paper[[6]])
    # 输出到新的CSV文件
    write.csv(df_d_paper[[6]], "../1_Clean_Data/d_prime/Schaefer_2019_CP_Exp1_d'.csv", row.names = FALSE)
    # 显示数据总结
    summary(df[[6]]) 
    rm(d_primes,subset_data,correct_rejection,d_prime,false_alarm,false_alarm_rate,hit,hit_rate,identities,identity,miss,z_false_alarm,z_hit,subject,subjects)
```
...
df[[7]] <- read.csv(
  "../1_Clean_Data/Schaefer_2019_CP/Schaefer_2019_CP_Exp2_Clean.csv"
  ) %>%
  # 添加新的环境变量
  dplyr::mutate(
    ScreenResolution = "1680*1050", # 屏幕分辨率
...
d_primes <- setNames(data.frame(Identity = character(), d_prime = numeric()), names(identities))
  ## 计算整篇文章的d'
    ...
    # 更新列名
    names(df)[8] <- "Schaefer_2019_CP_Exp3"
  ...
  ##计算每个被试的d'
    ...
    print(df_d_paper[[8]])
    # 输出到新的CSV文件
    write.csv(df_d_paper[[8]], "../1_Clean_Data/d_prime/Schaefer_2019_CP_Exp3_d'.csv", row.names = FALSE)
    # 显示数据总结
    summary(df[[8]]) 
    rm(d_primes,subset_data,correct_rejection,d_prime,false_alarm,false_alarm_rate,hit,hit_rate,identities,identity,miss,z_false_alarm,z_hit,subject,subjects)
```
