...
# 添加新的环境变量
  dplyr::mutate(
    ScreenResolution = NA, # 屏幕分辨率未提及
    RefreshRate = NA, # 刷新率未提及
    Luminance = NA, # 亮度未提及
    VisualAngle = NA, # 视觉角度未提及
    Color = NA, # 颜色未提及
    Contrast = NA, # 对比度未提及
    ScreenGamma = NA # 屏幕γ值未提及
  ) %>%
  dplyr::select(
...
# 添加新的环境变量
  dplyr::mutate(
    ScreenResolution = "1920 x 1080", # 屏幕分辨率
    ...
    Color = NA, # 文档中没有提及具体颜色
    Contrast = NA, # 文档中没有提及对比度
    ScreenGamma = NA, # 屏幕γ值未提及
  ) %>%
  dplyr::select(
...
# 添加新的环境变量
  dplyr::mutate(
    ScreenResolution = "17-in cathode-ray tube (CRT) monitor", # 文档中没有提及屏幕分辨率
...
df[[5]] <- read.csv(
  ...
  dplyr::mutate(
    ...
    Luminance = NA, # 亮度
    TextColor = NA, # 文档中没有提及具体颜色
    VisualAngle = NA, # 视觉角度，假设57厘米为视距，具体数值需要根据实验设置调整
    Color = NA, # 文档中没有提及具体颜色
    Contrast = NA # 文档中没有提及对比度
  ) %>%
  dplyr::arrange(
...
df[[6]] <- read.csv(
  ...
  # 添加新的环境变量
  dplyr::mutate(
    ScreenResolution = "1680 x 1050", # 屏幕分辨率
    RefreshRate = "60 Hz", # 刷新率，直接使用文本输出
    LuminanceBackground = NA, # 文档中没有提及背景亮度
    LuminanceStrongDark = NA, # 文档中没有提及强暗亮度
    LuminanceWeakDark = NA, # 文档中没有提及弱暗亮度
    LuminanceWeakLight = NA, # 文档中没有提及弱光亮度
    LuminanceStrongLight = NA,  # 文档中没有提及强光亮度
    TextColor = NA, # 文档中没有提及具体颜色
    VisualAngle = "0.48°", # 视觉角度，根据文档中提供的信息
    Color = NA, # 文档中没有提及具体颜色
    Contrast = NA # 文档中没有提及对比度
  ) %>%
  dplyr::select(
...
df[[9]] <- read.csv(
  ...
  dplyr::mutate(
   ...
   dplyr::mutate(
    ...
    VisualAngle = NA, # 视觉角度，根据文档中提供的信息
    Contrast = NA # 文档中没有提及对比度
  ) %>%
  dplyr::arrange(
    Subject, Matching, Identity
