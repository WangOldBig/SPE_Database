...
# 读取CSV文件
data <- read.csv("../2_Raw_Data/Kirk_2025_BJP/Kirk_2025_BJP_Exp2_raw.csv")
...
df[[55]] <- read.csv(
  ...
  dplyr::mutate(
    Matching = case_when(
      ...
      Matching == "mismatching" ~ "Nomatching",
    ),
    RT_ms = as.numeric(RT_ms),
    RT_sec = RT_ms / 1000,
    ) %>%
...
summary(df[[57]])
write.csv(x = df[[57]], file = "../1_Clean_Data/Kirk_2025_BJP/Kirk_2025_BJP_Exp1_Clean.csv")
```
