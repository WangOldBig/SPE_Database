...
# 读取CSV文件
data <- read.csv("../2_Raw_Data/Schäfer_2019_CP/Schäfer_2019_CP_Exp2_raw.csv")
...
df[[2]] <- read.csv(
  ...
  dplyr::select(
    ...
    Shape, Matching, Identity, Response, ShapeLoc, Colour, ShapeColour, textColour,
...
df[[9]] <- read.csv(
  ...
  dplyr::mutate(
    ...
    Identity = factor(
      Identity, levels = c("You", "Friend", "Stranger")
    ),
...
names(df)[11] <- "Golubickis_2021_AP Exp 2"
summary(df[[11]])
write.csv(x = df[[11]], file = "../1_Clean_Data/Golubickis_2021_AP/Golubickis_2021_AP_Exp2_Clean.csv")
```
