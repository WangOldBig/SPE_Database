...
# 读取CSV文件
data <- read.csv("../2_Raw_Data/Schäfer_2019_CP/Schäfer_2019_CP_Exp1_raw.csv")
...
df[[4]] <- read.csv(
  ...
  dplyr::select(
    Subject, Identity, Matching, Mood, Arouse, Valence, Session,  
    RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
...
df[[5]] <- read.csv(
  ...
  dplyr::select(
    Subject, Sex, Identity, Matching, Cue, RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
