...
df[[13]] <- read.csv(
  ...
  dplyr::select(
    Subject, Gender, Age, Handedness, First_Lanugage,
    Shape, Label, Matching, Identity, Response, RT_ms, RT_sec, ACC
  ) %>%
  dplyr::arrange(
    Subject, Matching, Identity
  )
...
df[[21]] <- read.csv(
  ...
  dplyr::select(
    ...
    Shape, Label, Matching, Identity,RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
...
```{r Paper t9 Experiment 1}
##################################Clean Data####################################
df[[30]] <- read.csv("../2_Raw_Data/Woźniak_2022_PR/Woźniak_2022_PR_Exp1_raw.csv") %>%
  dplyr::select(
    Subject = V1,
    Gender = V2,
    Shape = V7,
...
names(df)[30] <- "Woźniak_2022_PR_Exp 1"
summary(df[[30]])
write.csv(x = df[[30]], file = "../1_Clean_Data/Woźniak_2022_PR/Woźniak_2022_PR_Exp1_Clean.csv")
```
# Woźniak_2022_PR
## Experiment 2 (Identity = 3)
"Self", "Friend", "None"
...
```{r Paper t9 Experiment 2}
##################################Clean Data####################################
df[[31]] <- read.csv("../2_Raw_Data/Woźniak_2022_PR/Woźniak_2022_PR_Exp2_raw.csv") %>%
  dplyr::select(
    Subject = V1,
    Gender = V2,
    Shape = V7,
...
names(df)[31] <- "Woźniak_2022_PR_Exp 2"
summary(df[[31]])
write.csv(x = df[[31]], file = "../1_Clean_Data/Woźniak_2022_PR/Woźniak_2022_PR_Exp2_Clean.csv")
```
...
```{r Paper t9 Experiment 3}
##################################Clean Data####################################
df[[32]] <- read.csv("../2_Raw_Data/Woźniak_2022_PR/Woźniak_2022_PR_Exp3_raw.csv") %>%
  dplyr::select(
    Subject = V1,
    Gender = V2,
    Shape = V7,
...
names(df)[32] <- "Woźniak_2022_PR_Exp 3"
summary(df[[32]])
write.csv(x = df[[32]], file = "../1_Clean_Data/Woźniak_2022_PR/Woźniak_2022_PR_Exp3_Clean.csv")
```
...
names(df)[44] <- "Zhang_2022_NI"
summary(df[[44]])
write.csv(x = df[[44]], file = "../1_Clean_Data/Zhang_2022_NI/Zhang_2022_NI_Exp1_Clean.csv")
```
...
summary(df[[45]])
write.csv(x = df[[45]], file = "../1_Clean_Data/Zhang_2022_NI/Zhang_2022_NI_Exp2_Clean.csv")
```
...
summary(df[[46]])
write.csv(x = df[[46]], file = "../1_Clean_Data/Zhang_2022_NI/Zhang_2022_NI_Exp3_Clean.csv")
```
...
names(df)[48] <- "Martínez-Pérez_2024_CC"
summary(df[[48]])
write.csv(x = df[[48]], file = "../1_Clean_Data/Martínez-Pérez_2024_CC/Martínez-Pérez_2024_CC_Exp2_Clean.csv")
```
...
```{r Paper p6 Experiment 1}
df[[49]] <- read.csv(
  "../2_Raw_Data/Sui_2016_EPHPP/Merge_Data.csv"
  ) %>%
  dplyr::select(
    Subject = Subject,
    Age= Age,
...
names(df)[49] <- "Sui_2016_EPHPP"
summary(df[[49]])
write.csv(x = df[[49]], file = "../1_Clean_Data/Sui_2016_EPHPP/Sui_2016_EPHPP_Exp1_Clean.csv")
```
...
df[[50]] <- read.csv(
  ...
  dplyr::select(
    Subject, Age, Gender,  
    Matching, Identity, Session, RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
...
df[[51]] <- read.mat(
  ...
  dplyr::select(
    Subject, Age,Handedness, Matching, Identity, Session, RT_ms, RT_sec, ACC
  ) %>%
  dplyr::mutate(
    RT_sec = round(RT_sec, 3),
    RT_ms = round(RT_ms, 0)
...
# 定义路径
source_path <- "../2_Raw_Data/Sui_2015/Source/"
raw_path <- "../2_Raw_Data/Sui_2015/Raw/"

# 获取所有.mat文件
mat_files <- list.files(source_path, pattern = "\\.mat$", full.names = TRUE)
...
for (file in mat_files) {
  ...
  # 打印读取的数据结构，确保我们知道MAT文件内部的结构
  print(str(data))
  
  # 尝试提取MAT文件中的第一个数据结构
  # 注意：你需要根据实际的MAT文件结构调整这里的代码
  if (length(data) > 0) {
    # 假设MAT文件中第一个非元数据对象为所需数据
    first_data <- data[[1]]
    
    # 检查是否为数据框或矩阵
    if (is.matrix(first_data) || is.data.frame(first_data)) {
      # 将其转换为数据框（若有需要）
      df <- as.data.frame(first_data)
      
      # 获取文件名（去掉路径和扩展名）
      file_name <- tools::file_path_sans_ext(basename(file))
      
      # 定义.csv文件的保存路径
      csv_file <- file.path(raw_path, paste0(file_name, ".csv"))
      
      # 写入.csv文件
      write.csv(df, csv_file, row.names = FALSE)
    } else {
      cat("Warning: The first object in MAT file", file, "is not a matrix or data frame. Skipping.\n")
    }
  } else {
    cat("Warning: No data found in MAT file", file, ". Skipping.\n")
  }
}
...
for (file in mat_files) {
  ...
  # 假设我们提取以下几个变量：expe, num, sub, sex, age, ses
  df <- data.frame(
    Experiment = as.character(data$expe),
    Subject_Number = as.character(data$num),
    Subject_Initials = as.character(data$sub),
    Gender = as.character(data$sex),
    Age = as.character(data$age),
    Session = as.character(data$ses)
  )
