...
df[[1]] <- read.csv(
  "../1_Clean_Data/Constable_2020_AP/Constable_2020_AP_Exp1_Clean.csv"
) %>%
  # 添加新的环境变量
  dplyr::mutate(
    ...
    EEG_Data = 0,
    Stimu_language = "Hungarian"
  ) %>%
  dplyr::arrange(Subject, Matching, ShapeLoc)

# 为P1, P2, Stranger分别计算d'
identities <- c("P1", "P2", "Stranger")

  ## 计算整篇文章的d'
    # 创建一个新的d_prime数据框，用于保存d_prime值
    df_d_prime[[1]] <- data.frame(Identity = character(), d_prime = numeric())
    
    # 计算d'
    for (identity in identities) {
      # 提取当前Identity的数据
      subset_data <- df[[1]] %>% filter(Identity == identity)
      ...
      # 保存计算结果到df_d_prime[[1]]
      df_d_prime[[1]] <- rbind(df_d_prime[[1]], data.frame(Identity = identity, d_prime = d_prime))
    }
    
    # 打印df_d_prime[[1]]，包含所有Identity的d'值
    print(df_d_prime[[1]])
    
    # 更新列名
    names(df)[1] <- "Constable_2020_AP_Exp1"
  ...
  ##计算每个被试的d'
    ...
    # 创建一个新的d_prime数据框，用于保存d_prime值
    df_d_paper[[1]] <- data.frame(Subject = character(), Identity = character(), d_prime = numeric())
    
    # 计算d'
    for (identity in identities) {
      subjects <- unique(df[[1]]$Subject)  # 获取所有被试
    
      for (subject in subjects) {
        # 提取当前Identity和Subject的数据
        subset_data <- df[[1]] %>% filter(Identity == identity & Subject == subject)
        ...
        # 计算Hit Rate和False Alarm Rate
        hit_rate <- ifelse(hit + miss > 0, hit / (hit + miss), 0)
        false_alarm_rate <- ifelse(false_alarm + correct_rejection > 0, 
                                   false_alarm / (false_alarm + correct_rejection), 
                                   0)
        ...
        # 保存计算结果到df_d_paper[[3]]
        df_d_paper[[1]] <- rbind(df_d_paper[[1]], data.frame(Subject = subject, Identity = identity, d_prime = d_prime))
      }
    }
    
    # 打印df_d_paper[[1]]，包含所有Subject和Identity的d'值
    print(df_d_paper[[1]])
    
    # 输出到新的CSV文件
    write.csv(df_d_paper[[1]], "../1_Clean_Data/d_prime/Constable_2020_AP_d'.csv", row.names = FALSE)
...
df[[3]] <- read.csv(
  ...
  ## 计算整篇文章的d'
    # 创建一个新的d_prime数据框，用于保存d_prime值
    df_d_prime[[3]] <- data.frame(Identity = character(), d_prime = numeric())
    
    # 计算d'
    for (identity in identities) {
      # 提取当前Identity的数据
      subset_data <- df[[3]] %>% filter(Identity == identity)
      ...
      # 保存计算结果到df_d_prime[[3]]
      df_d_prime[[3]] <- rbind(df_d_prime[[3]], data.frame(Identity = identity, d_prime = d_prime))
    }
    
    # 打印df_d_prime[[3]]，包含所有Identity的d'值
    print(df_d_prime[[3]])
    
    # 更新列名
    names(df)[3] <- "Constable_2020_CE_Exp2"

  ##计算每个被试的d'
    d_primes <- setNames(data.frame(Subject = character(), Identity = character(), d_prime = numeric()), c("Subject", "Identity", "d_prime"))
    
    # 创建一个新的d_prime数据框，用于保存d_prime值
    df_d_paper[[3]] <- data.frame(Subject = character(), Identity = character(), d_prime = numeric())
    
    # 计算d'
    for (identity in identities) {
      subjects <- unique(df[[3]]$Subject)  # 获取所有被试
    
      for (subject in subjects) {
        # 提取当前Identity和Subject的数据
        subset_data <- df[[3]] %>% filter(Identity == identity & Subject == subject)
        ...
        d_primes <- rbind(d_primes, data.frame(Subject = subject, Identity = identity, d_prime = d_prime))
        # 保存计算结果到df_d_paper[[3]]
        df_d_paper[[3]] <- rbind(df_d_paper[[3]], data.frame(Subject = subject, Identity = identity, d_prime = d_prime))
      }
    }
    
    # 打印df_d_paper[[3]]，包含所有Subject和Identity的d'值
    print(df_d_paper[[3]])
    
    # 输出到新的CSV文件
    write.csv(df_d_paper[[3]], "../1_Clean_Data/d_prime/Constable_2020_CE_Exp2_d'.csv", row.names = FALSE)
...
# 为Self, Familiar, Stranger分别计算d'
  identities <- c("Self", "Familiar", "Stranger")

  ## 计算整篇文章的d'
    # 创建一个新的d_prime数据框，用于保存d_prime值
    df_d_prime[[4]] <- data.frame(Identity = character(), d_prime = numeric())
    
    # 计算d'
    for (identity in identities) {
      # 提取当前Identity的数据
      subset_data <- df[[4]] %>% filter(Identity == identity)
      ...
      # 保存计算结果到df_d_prime[[4]]
      df_d_prime[[4]] <- rbind(df_d_prime[[4]], data.frame(Identity = identity, d_prime = d_prime))
    }
    
    # 打印df_d_prime[[4]]，包含所有Identity的d'值
    print(df_d_prime[[4]])
    
    # 更新列名
    names(df)[4] <- "Qian_2019_QJEP_Exp1"

  ##计算每个被试的d'
    d_primes <- setNames(data.frame(Subject = character(), Identity = character(), d_prime = numeric()), c("Subject", "Identity", "d_prime"))
    
    # 创建一个新的d_prime数据框，用于保存d_prime值
    df_d_paper[[4]] <- data.frame(Subject = character(), Identity = character(), d_prime = numeric())
    
    # 计算d'
    for (identity in identities) {
      subjects <- unique(df[[4]]$Subject)  # 获取所有被试
    
      for (subject in subjects) {
        # 提取当前Identity和Subject的数据
        subset_data <- df[[4]] %>% filter(Identity == identity & Subject == subject)
        ...
        d_primes <- rbind(d_primes, data.frame(Subject = subject, Identity = identity, d_prime = d_prime))
        # 保存计算结果到df_d_paper[[4]]
        df_d_paper[[4]] <- rbind(df_d_paper[[4]], data.frame(Subject = subject, Identity = identity, d_prime = d_prime))
      }
    }
    
    # 打印df_d_paper[[4]]，包含所有Subject和Identity的d'值
    print(df_d_paper[[4]])
    
    # 输出到新的CSV文件
    write.csv(df_d_paper[[4]], "../1_Clean_Data/d_prime/Qian_2019_QJEP_Exp1_d'.csv", row.names = FALSE)
    # 显示数据总结
    summary(df[[4]])
    rm(d_primes,subset_data,correct_rejection,d_prime,false_alarm,false_alarm_rate,hit,hit_rate,identities,identity,miss,z_false_alarm,z_hit,subject,subjects)
```
...
# 为Self, Familiar分别计算d'
  identities <- c("Self", "Familiar")

  ## 计算整篇文章的d'
    # 创建一个新的d_prime数据框，用于保存d_prime值
    df_d_prime[[5]] <- data.frame(Identity = character(), d_prime = numeric())
    
    # 计算d'
    for (identity in identities) {
      # 提取当前Identity的数据
      subset_data <- df[[5]] %>% filter(Identity == identity)
      ...
      # 保存计算结果到df_d_prime[[5]]
      df_d_prime[[5]] <- rbind(df_d_prime[[5]], data.frame(Identity = identity, d_prime = d_prime))
    }
    
    # 打印df_d_prime[[5]]，包含所有Identity的d'值
    print(df_d_prime[[5]])
    
    # 更新列名
    names(df)[5] <- "Qian_2019_QJEP_Exp2"

  ##计算每个被试的d'
    d_primes <- setNames(data.frame(Subject = character(), Identity = character(), d_prime = numeric()), c("Subject", "Identity", "d_prime"))
    
    # 创建一个新的d_prime数据框，用于保存d_prime值
    df_d_paper[[5]] <- data.frame(Subject = character(), Identity = character(), d_prime = numeric())
    
    # 计算d'
    for (identity in identities) {
      subjects <- unique(df[[5]]$Subject)  # 获取所有被试
    
      for (subject in subjects) {
        # 提取当前Identity和Subject的数据
        subset_data <- df[[5]] %>% filter(Identity == identity & Subject == subject)
        ...
        d_primes <- rbind(d_primes, data.frame(Subject = subject, Identity = identity, d_prime = d_prime))
        # 保存计算结果到df_d_paper[[5]]
        df_d_paper[[5]] <- rbind(df_d_paper[[5]], data.frame(Subject = subject, Identity = identity, d_prime = d_prime))
      }
    }
    
    # 打印df_d_paper[[5]]，包含所有Subject和Identity的d'值
    print(df_d_paper[[5]])
        # 输出到新的CSV文件
    write.csv(df_d_paper[[5]], "../1_Clean_Data/d_prime/Qian_2019_QJEP_Exp2_d'.csv", row.names = FALSE)
    # 显示数据总结
    summary(df[[5]]) 
    rm(d_primes,subset_data,correct_rejection,d_prime,false_alarm,false_alarm_rate,hit,hit_rate,identities,identity,miss,z_false_alarm,z_hit,subject,subjects)
```
...
df[[6]] <- read.csv(
  "../1_Clean_Data/Schäfer_2019_CP/Schäfer_2019_CP_Exp1_Clean.csv"
  ) %>%
  # 添加新的环境变量
  dplyr::mutate(
...
# 为Self, Friend，Stranger分别计算
  identities <- c("Self","Mother", "Acquaintance","none")
  d_primes <- setNames(data.frame(Identity = character(), d_prime = numeric()), names(identities))
    ## 计算整篇文章的d'
    # 创建一个新的d_prime数据框，用于保存d_prime值
    df_d_prime[[6]] <- data.frame(Identity = character(), d_prime = numeric())
    
    # 计算d'
    for (identity in identities) {
      # 提取当前Identity的数据
      subset_data <- df[[6]] %>% filter(Identity == identity)
      ...
      # 保存计算结果到df_d_prime[[6]]
      df_d_prime[[6]] <- rbind(df_d_prime[[6]], data.frame(Identity = identity, d_prime = d_prime))
    }
    
    # 打印df_d_prime[[6]]，包含所有Identity的d'值
    print(df_d_prime[[6]])
    
    # 更新列名
    names(df)[6] <- "Schäfer_2019_CP_Exp1"

  ##计算每个被试的d'
    d_primes <- setNames(data.frame(Subject = character(), Identity = character(), d_prime = numeric()), c("Subject", "Identity", "d_prime"))
    
    # 创建一个新的d_prime数据框，用于保存d_prime值
    df_d_paper[[6]] <- data.frame(Subject = character(), Identity = character(), d_prime = numeric())
    
    # 计算d'
    for (identity in identities) {
      subjects <- unique(df[[6]]$Subject)  # 获取所有被试
      
      for (subject in subjects) {
        # 提取当前Identity和Subject的数据
        subset_data <- df[[6]] %>% filter(Identity == identity & Subject == subject)
        ...
        d_primes <- rbind(d_primes, data.frame(Subject = subject, Identity = identity, d_prime = d_prime))
        # 保存计算结果到df_d_paper[[6]]
        df_d_paper[[6]] <- rbind(df_d_paper[[6]], data.frame(Subject = subject, Identity = identity, d_prime = d_prime))
      }
    }
    
    # 打印df_d_paper[[6]]，包含所有Subject和Identity的d'值
    print(df_d_paper[[6]])
    # 输出到新的CSV文件
    write.csv(df_d_paper[[6]], "../1_Clean_Data/d_prime/Schäfer_2019_CP_Exp1_d'.csv", row.names = FALSE)
    # 显示数据总结
    summary(df[[6]]) 
    rm(d_primes,subset_data,correct_rejection,d_prime,false_alarm,false_alarm_rate,hit,hit_rate,identities,identity,miss,z_false_alarm,z_hit,subject,subjects)
```
...
df[[7]] <- read.csv(
  "../1_Clean_Data/Schäfer_2019_CP/Schäfer_2019_CP_Exp2_Clean.csv"
  ) %>%
  # 添加新的环境变量
  dplyr::mutate(
    ScreenResolution = "1680*1050", # 屏幕分辨率
...
df[[8]] <- read.csv(
  "../1_Clean_Data/Schäfer_2019_CP/Schäfer_2019_CP_Exp3_Clean.csv"
  ) %>%
  # 添加新的环境变量
  dplyr::mutate(
    ScreenResolution = "1680*1050", # 屏幕分辨率
...
# 为Self, Friend，Stranger分别计算
identities <- c("Self", "Mother", "Acquaintance")
d_primes <- setNames(data.frame(Identity = character(), d_prime = numeric()), names(identities))
  ## 计算整篇文章的d'
    # 创建一个新的d_prime数据框，用于保存d_prime值
    df_d_prime[[8]] <- data.frame(Identity = character(), d_prime = numeric())
    
    # 计算d'
    for (identity in identities) {
      # 提取当前Identity的数据
      subset_data <- df[[8]] %>% filter(Identity == identity)
      ...
      # 保存计算结果到df_d_prime[[8]]
      df_d_prime[[8]] <- rbind(df_d_prime[[8]], data.frame(Identity = identity, d_prime = d_prime))
    }
    
    # 打印df_d_prime[[8]]，包含所有Identity的d'值
    print(df_d_prime[[8]])
    
    # 更新列名
    names(df)[8] <- "Schäfer_2019_CP_Exp3"
    
  ##计算每个被试的d'
    d_primes <- setNames(data.frame(Subject = character(), Identity = character(), d_prime = numeric()), c("Subject", "Identity", "d_prime"))
    
    # 创建一个新的d_prime数据框，用于保存d_prime值
    df_d_paper[[8]] <- data.frame(Subject = character(), Identity = character(), d_prime = numeric())
    
    # 计算d'
    for (identity in identities) {
      subjects <- unique(df[[8]]$Subject)  # 获取所有被试
      
      for (subject in subjects) {
        # 提取当前Identity和Subject的数据
        subset_data <- df[[8]] %>% filter(Identity == identity & Subject == subject)
        ...
        d_primes <- rbind(d_primes, data.frame(Subject = subject, Identity = identity, d_prime = d_prime))
        # 保存计算结果到df_d_paper[[8]]
        df_d_paper[[8]] <- rbind(df_d_paper[[8]], data.frame(Subject = subject, Identity = identity, d_prime = d_prime))
      }
    }
    
    # 打印df_d_paper[[8]]，包含所有Subject和Identity的d'值
    print(df_d_paper[[8]])
    # 输出到新的CSV文件
    write.csv(df_d_paper[[8]], "../1_Clean_Data/d_prime/Schäfer_2019_CP_Exp3_d'.csv", row.names = FALSE)
    # 显示数据总结
    summary(df[[8]]) 
    rm(d_primes,subset_data,correct_rejection,d_prime,false_alarm,false_alarm_rate,hit,hit_rate,identities,identity,miss,z_false_alarm,z_hit,subject,subjects)
```
...
df[[9]] <- read.csv(
  "../1_Clean_Data/Woźniak_2018_PLOS/Woźniak_2018_PLOS_Exp1_Clean.csv"
  ) %>%
   dplyr::mutate(
...
# 为Self, Friend，Stranger分别计算
  ...
  ## 计算整篇文章的d'
  # 创建一个新的d_prime数据框，用于保存d_prime值
  df_d_prime[[9]] <- data.frame(Identity = character(), d_prime = numeric())
  
  # 计算d'
  for (identity in identities) {
    # 提取当前Identity的数据
    subset_data <- df[[9]] %>% filter(Identity == identity)
    ...
    # 保存计算结果到df_d_prime[[9]]
    df_d_prime[[9]] <- rbind(df_d_prime[[9]], data.frame(Identity = identity, d_prime = d_prime))
  }
  
  # 打印df_d_prime[[9]]，包含所有Identity的d'值
  print(df_d_prime[[9]])
  
  # 更新列名
  names(df)[9] <- "Woźniak_2018_PLOS_Exp1"
  
  ##计算每个被试的d'
  d_primes <- setNames(data.frame(Subject = character(), Identity = character(), d_prime = numeric()), c("Subject", "Identity", "d_prime"))
  
  # 创建一个新的d_prime数据框，用于保存d_prime值
  df_d_paper[[9]] <- data.frame(Subject = character(), Identity = character(), d_prime = numeric())
  
  # 计算d'
  for (identity in identities) {
    subjects <- unique(df[[9]]$Subject)  # 获取所有被试
    
    for (subject in subjects) {
      # 提取当前Identity和Subject的数据
      subset_data <- df[[9]] %>% filter(Identity == identity & Subject == subject)
      ...
      d_primes <- rbind(d_primes, data.frame(Subject = subject, Identity = identity, d_prime = d_prime))
      # 保存计算结果到df_d_paper[[9]]
      df_d_paper[[9]] <- rbind(df_d_paper[[9]], data.frame(Subject = subject, Identity = identity, d_prime = d_prime))
    }
  }
  
  # 打印df_d_paper[[9]]，包含所有Subject和Identity的d'值
  print(df_d_paper[[9]])
  # 输出到新的CSV文件
  write.csv(df_d_paper[[9]], "../1_Clean_Data/d_prime/Woźniak_2018_PLOS_Exp1_d'.csv", row.names = FALSE)
  # 显示数据总结
  summary(df[[9]]) 
  rm(d_primes,subset_data,correct_rejection,d_prime,false_alarm,false_alarm_rate,hit,hit_rate,identities,identity,miss,z_false_alarm,z_hit,subject,subjects)
```
...
# 为Self, Friend，Stranger分别计算
  ...
  for (identity in identities) {
    ...
    # 保存结果
    d_primes <- rbind(d_primes, data.frame(Identity = identity, d_prime = d_prime))
...
summary(df[[11]])
rm(d_primes,subset_data,correct_rejection,d_prime,false_alarm,false_alarm_rate,hit,hit_rate,identities,identity,miss,z_false_alarm,z_hit)
```
